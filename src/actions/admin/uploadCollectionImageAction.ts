...
export async function uploadCollectionImageAction(formData: FormData): Promise<{
  success: boolean
  url?: string
  storage_path?: string
  error?: string
  ...
  try {
    ...
    const hasPermission = await checkUserPermission(adminSession.email, 'manage_collections')
    if (!hasPermission) {
      return { success: false, error: 'Access Denied: You do not have permission to manage collections.' }
    }
    ...
    if (file.type === 'image/svg+xml' || fileExt === '.svg') {
      return { success: false, error: 'SVG vector images are not allowed for collection photos due to security guidelines. Please upload JPG, PNG, WebP, AVIF, HEIC, or GIF image files.' }
    }
    ...
    if (error || !data) {
      console.error('[uploadCollectionImageAction] Supabase storage upload failed:', error)
    ...
    console.error('[uploadCollectionImageAction] Unexpected error:', err)
