...
export async function uploadProductImageAction(formData: FormData): Promise<{
  ...
  try {
    ...
    const hasPermission = await checkUserPermission(adminSession.email, 'manage_products')
    if (!hasPermission) {
    ...
    if (file.type === 'image/svg+xml' || fileExt === '.svg') {
      return { success: false, error: 'SVG vector images are not allowed for product photos due to security guidelines. Please upload JPG, PNG, WebP, AVIF, HEIC, or GIF image files.' }
    }
    ...
    const pathPrefix = productId && productId.trim() !== '' ? `products/${productId.trim()}` : 'products/temp'
    ...
    if (error || !data) {
      console.error('[uploadProductImageAction] Supabase storage upload failed:', error)
    ...
    console.error('[uploadProductImageAction] Unexpected error:', err)
