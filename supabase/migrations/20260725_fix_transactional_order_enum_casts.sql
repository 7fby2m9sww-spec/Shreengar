-- Migration: Fix Transactional Order Flow Custom Enum Type Casts (Sprint 3.2.3)
-- Description: Cast VARCHAR status values and parameters explicitly to custom enum types to prevent casting mismatches on remote Supabase DB.

BEGIN;

-- 1. Recreate place_order_atomic with explicit casts
...
BEGIN
  ...
  -- Rule: Free shipping for all orders (temporary business rule).
  v_shipping_charge := 0;
  ...
  LOOP
    ...
    SET 
      quantity = v_new_qty,
      stock_status = CASE 
        WHEN v_new_qty > 0 THEN 'in_stock'::public.inventory_status
        ELSE 'out_of_stock'::public.inventory_status
      END,
      updated_at = NOW()
    WHERE id = v_inv_id;
...
-- 2. Recreate update_order_status_atomic with explicit casts
