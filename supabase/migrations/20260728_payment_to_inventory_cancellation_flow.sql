-- Migration: Payment-to-Inventory and pre-dispatch cancellation flow (Sprint 3.3.1)
-- Description: Redefines place_order_atomic to skip stock deduction at checkout.
-- Defines deduct_inventory_for_order and confirm_payment_and_deduct_inventory_atomic to handle atomic stock deduction at payment verification.

BEGIN;

-- 1. Recreate place_order_atomic with stock validation only (no deduction)
...
BEGIN
  ...
  SELECT 
    COALESCE(NULLIF(TRIM(full_name), ''), 'Customer'), email, phone
  INTO
  ...
  INSERT INTO public.orders (
    ...
    'pending'::public.order_status, 'pending'::public.payment_status, p_payment_method::public.payment_method
  ...
  -- 12. Loop through cart items to create order items and validate stock ONLY (no deduction yet)
  FOR v_item IN 
    SELECT (val->>'variant_id')::UUID AS variant_id, SUM((val->>'quantity')::INT)::INT AS quantity
    FROM jsonb_array_elements(p_cart_items) AS val
    GROUP BY (val->>'variant_id')
  LOOP
    ...
    IF v_current_qty IS NULL OR v_current_qty < v_qty THEN
      RAISE EXCEPTION 'Insufficient stock for product "%". Only % left in stock.', v_product_name, COALESCE(v_current_qty, 0);
    END IF;
...
-- 2. Define deduct_inventory_for_order function
...
BEGIN
  ...
  FOR v_item IN 
    SELECT oi.id, oi.variant_id, oi.quantity, oi.product_name, oi.sku
    FROM public.order_items oi
    WHERE oi.order_id = p_order_id
  LOOP
    -- Unique operation key for idempotency
    v_op_key := 'checkout:' || p_order_id::TEXT || ':' || v_item.id::TEXT;

    -- If this item has already been deducted, skip it
    IF NOT EXISTS (SELECT 1 FROM public.inventory_transactions WHERE operation_key = v_op_key) THEN
      -- Lock inventory row
      SELECT id, quantity, reserved_quantity, low_stock_threshold 
      INTO v_inv_id, v_current_qty, v_reserved_qty, v_low_stock_threshold
      FROM public.inventory
      WHERE variant_id = v_item.variant_id
      FOR UPDATE;

      IF v_inv_id IS NULL THEN
        RAISE EXCEPTION 'Inventory record not found for variant SKU %', v_item.sku;
      END IF;

      -- Check stock availability at capture time
      IF v_current_qty < v_item.quantity THEN
        RAISE EXCEPTION 'Insufficient stock for product "%". Only % left in stock.', v_item.product_name, v_current_qty;
      END IF;

      v_new_qty := v_current_qty - v_item.quantity;
      v_available_qty := GREATEST(v_new_qty - COALESCE(v_reserved_qty, 0), 0);
      ...
      SET 
        quantity = v_new_qty,
        stock_status = CASE
          WHEN v_available_qty <= 0 THEN 'out_of_stock'::public.inventory_status
          WHEN v_available_qty <= COALESCE(v_low_stock_threshold, 5) THEN 'low_stock'::public.inventory_status
      ...
      INSERT INTO public.inventory_transactions (
        ...
        v_inv_id, v_item.variant_id, v_current_qty, v_new_qty, -v_item.quantity, 'Order Payment Confirmation', p_order_id, v_item.id, v_op_key, NOW()
      );
    END IF;
  END LOOP;
...
-- 3. Define confirm_payment_and_deduct_inventory_atomic function
CREATE OR REPLACE FUNCTION public.confirm_payment_and_deduct_inventory_atomic(
  p_order_id UUID,
...
BEGIN
  -- Lock the order row
  SELECT status, payment_status, total_amount INTO v_status, v_pay_status, v_total
  FROM public.orders
  WHERE id = p_order_id
  FOR UPDATE;
  ...
  -- Deduct inventory
  v_res := public.deduct_inventory_for_order(p_order_id);
...
REVOKE EXECUTE ON FUNCTION public.confirm_payment_and_deduct_inventory_atomic(UUID, VARCHAR, VARCHAR) FROM public, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.deduct_inventory_for_order(UUID) TO service_role;
GRANT EXECUTE ON FUNCTION public.confirm_payment_and_deduct_inventory_atomic(UUID, VARCHAR, VARCHAR) TO service_role;
