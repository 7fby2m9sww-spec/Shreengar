-- Migration: Fix Atomic Inventory low_stock Status (Sprint 3.2.4 Additive)
-- Description: Redefines place_order_atomic and update_order_status_atomic to select and use low_stock_threshold and reserved_quantity to compute and assign stock_status correctly.

BEGIN;

-- 1. Recreate place_order_atomic with corrected status calculation
...
BEGIN
  ...
  LOOP
    ...
    -- Lock inventory row using SELECT ... FOR UPDATE (selecting quantity, reserved_quantity, low_stock_threshold)
    ...
    -- Calculate available quantity consistently (GREATEST to avoid negative values)
    ...
    -- Insert inventory transaction row
...
-- 2. Recreate update_order_status_atomic with corrected status calculation
...
BEGIN
  ...
  -- 4. Restore stock if status becomes cancelled
  IF p_new_status = 'cancelled' THEN
    ...
    LOOP
      IF v_item.variant_id IS NOT NULL THEN
        ...
        -- Verify that stock was originally decremented and restoration doesn't exist (idempotency check)
        ...
        THEN
          -- Lock inventory row (selecting quantity, reserved_quantity, low_stock_threshold)
          SELECT id, quantity, reserved_quantity, low_stock_threshold 
          INTO v_inv_id, v_current_qty, v_reserved_qty, v_low_stock_threshold
          FROM public.inventory
