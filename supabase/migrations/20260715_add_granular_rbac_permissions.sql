-- =========================================================================================
-- MIGRATION A: SEED RBAC GRANULARITY & ROLE-PERMISSION MAPPINGS (PHASE 2)
-- File: supabase/migrations/20260715_add_granular_rbac_permissions.sql
-- Description: Idempotent seed for 32 granular view/manage permissions and mapping them
-- to known system roles. Preserves all legacy permissions and custom mappings.
-- =========================================================================================
...
BEGIN
    ...
    -- 3.2 ADMIN (Includes read access to system pages, no mutations)
    INSERT INTO public.role_permissions (role_id, permission_id)
    ...
    WHERE p.name IN (
        ...
        'view_reviews', 'manage_reviews', 'view_admin_users', 'view_roles', 'view_permissions', 'view_security', 'view_activity_logs'
    )
    ON CONFLICT (role_id, permission_id) DO NOTHING;
